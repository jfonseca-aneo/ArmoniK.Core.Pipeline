...
healthChecks:
  ...
  for i in {0..2}; do 
    ...
    echo -n "  startup: " && curl -sSL --retry 5 --retry-delay 1 --retry-connrefused localhost:998${i}/startup
    echo -n "  liveness: " && curl -sSL --retry 5 --retry-delay 1 --retry-connrefused localhost:998${i}/liveness
    echo -n "  readiness: " && curl -sSL --retry 5 --retry-delay 1 --retry-connrefused localhost:998${i}/readiness
  done

  echo -e "\nHealth Checking Submitter"
  echo -n "  startup: " && curl -sSL --retry 5 --retry-delay 1 --retry-connrefused localhost:5011/startup
  echo -n "  liveness: " && curl -sSL --retry 5 --retry-delay 1 --retry-connrefused localhost:5011/liveness
